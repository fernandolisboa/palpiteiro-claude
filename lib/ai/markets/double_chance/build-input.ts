import { DoubleChanceInputSchema, type DoubleChanceInput } from "./schemas";
...
): DoubleChanceInput["home"]["standing"] {
  return {
...
function buildFormMatches(
  ...
  teamName: string,
  limit: number,
): DoubleChanceInput["home"]["form"]["matches"] {
  const matches: DoubleChanceInput["home"]["form"]["matches"] = [];
...
): DoubleChanceInput["home"]["absences"] {
...
): DoubleChanceInput["home"]["lineup"] | undefined {
...
function buildH2H(
  ...
  limit: number,
): DoubleChanceInput["h2h"] {
  const matches: DoubleChanceInput["h2h"] = [];
  const sorted = [...fixtures].sort(
    (a, b) => b.kickoffTimestampMs - a.kickoffTimestampMs,
  );
  for (const f of sorted) {
...
// por selectionKey + pct map); buildPredictionInput DOWN-MAPEIA pro shape de
// 3 duplas do DoubleChanceInput.
...
export function buildPredictionInput(
  ...
  // Down-map do shape GENÉRICO (selections[]/pct) pras 3 duplas. As chaves
  // home_or_draw/away_or_draw/home_or_away vêm de descriptor.selectionKeys
  // (predict monta o generic args nessa ordem). Ausência = bug de seed/descriptor.
  const hdSel = args.odds.selections.find((s) => s.key === "home_or_draw");
  const adSel = args.odds.selections.find((s) => s.key === "away_or_draw");
  const haSel = args.odds.selections.find((s) => s.key === "home_or_away");
  if (!hdSel || !adSel || !haSel) {
    throw new BuildInputError(
      "double_chance odds missing 'home_or_draw'/'away_or_draw'/'home_or_away' selection in generic args",
      { keys: args.odds.selections.map((s) => s.key) },
    );
  }
  const hdPct = args.implied.pct.home_or_draw;
  const adPct = args.implied.pct.away_or_draw;
  const haPct = args.implied.pct.home_or_away;
  if (hdPct === undefined || adPct === undefined || haPct === undefined) {
    throw new BuildInputError(
      "double_chance implied missing 'home_or_draw'/'away_or_draw'/'home_or_away' pct in generic args",
      { keys: Object.keys(args.implied.pct) },
    );
  }

  const draft: DoubleChanceInput = {
    match: {
    ...
    odds: {
      ...
      home_or_draw_decimal: hdSel.odd,
      away_or_draw_decimal: adSel.odd,
      home_or_away_decimal: haSel.odd,
      captured_at: args.odds.captured_at,
    },
    implied: {
      home_or_draw_pct: hdPct,
      away_or_draw_pct: adPct,
      home_or_away_pct: haPct,
    },
  } satisfies DoubleChanceInput;

  const parsed = DoubleChanceInputSchema.safeParse(draft);
  if (!parsed.success) {
    throw new BuildInputError("DoubleChanceInputSchema validation failed", {
