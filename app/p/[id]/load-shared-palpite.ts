...
/**
 * Carrega o palpite compartilhado por id, com os 3 triggers de 404 (ADR 0035 §6):
 ...
 * imagem 404-fallback nos MESMOS 3 triggers que a página (não só "sem row").
 */
export const loadSharedPalpite = cache(
  async (id: string): Promise<SharedPalpite | null> => {
    ...
    const view = toPalpiteHeadlineViewFromSet(row.palpiteSetWithLines);
    if (!view) return null;
