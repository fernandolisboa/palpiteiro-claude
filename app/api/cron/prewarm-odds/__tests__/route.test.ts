...
vi.mock("@/lib/odds/prewarm-odds", () => ({
  prewarmOdds: vi.fn(),
}));

import { GET } from "@/app/api/cron/prewarm-odds/route";
import { prewarmOdds } from "@/lib/odds/prewarm-odds";

const runMock = vi.mocked(prewarmOdds);

function req(authorization?: string): Request {
  const headers = new Headers();
  if (authorization !== undefined) headers.set("authorization", authorization);
  return new Request("http://x/api/cron/prewarm-odds", { headers });
}
...
describe("GET /api/cron/prewarm-odds", () => {
  it("401 e loga quando CRON_SECRET não está setado, sem pré-aquecer", async () => {
  ...
  it("500 e loga quando o pré-aquecimento lança", async () => {
