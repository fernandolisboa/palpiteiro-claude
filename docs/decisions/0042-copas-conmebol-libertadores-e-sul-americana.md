# ADR 0042 — Copas CONMEBOL: Libertadores e Sul-Americana como ligas suportadas (registradas, fora de ACTIVE_LEAGUES)
...
> **Número provisório:** 0039–0041 estão tomados por PRs abertos (Fase C, CSP, JEV) e as threads irmãs de ligas também vão pedir número. Renumerar pro próximo livre no momento do merge.
...
3. **Registradas, mas fora de `ACTIVE_LEAGUES`.** O dono não vai subir o plano da The Odds API agora (500 créditos/mês). O prewarm custa ~2 créditos por liga por run (h2h + totals, featured) nos runs com jogo na janela, 4 runs/dia; cada copa em mata-mata soma algo como 40–60 créditos/mês (estimativa: 2–3 semanas de jogo/mês, 2–3 dias com jogo em 24h por semana), mais fetches de página e closing line. Com Brasileirão + Champions + Premier League + La Liga já perto do teto, ligar as copas estoura o free tier. No seletor elas aparecem **desabilitadas** (comportamento padrão de liga de clube inativa); ligar = uma linha em `ACTIVE_LEAGUES`.
